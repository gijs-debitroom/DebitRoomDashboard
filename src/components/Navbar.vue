...
export default {
    setup() {
      ...
      const { drawerHandler } = store;
      const isOpen = computed(() => store.isOpen);
      return { drawerHandler, isOpen };
    },
    methods: {
      toggle: function () {
        document.querySelector('#moon').classList.toggle('hidden');
        document.querySelector('#sun').classList.toggle('display');
...
<template>
  <section class="border-b border-gray-200 pt-5 px-6 bg-white shadow dark:bg-dark-pageBackgroundColor">
    <nav>
      <div class="pb-5">
        <div class="relative flex items-center">
          <div class="sm:hidden lg:block">
            <button  @click="drawerHandler" class="navbar-burger flex items-center text-gray-500 p-3">
          ...
          <div class="flex justify-end lg:hidden">
            <button class="flex items-center">
              <svg class="text-indigo-500 bg-indigo-100 block h-8 w-8 p-2 rounded" viewbox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" fill="currentColor">
                <title>Mobile menu</title>
                <path d="M0 3h20v2H0V3zm0 6h20v2H0V9zm0 6h20v2H0v-2z"></path>
              </svg>
            </button>
          </div>
          <ul class="hidden lg:flex lg:justify-end mr-6">
            <li>
              <button v-on:click="toggle" class="text-gray-200 hover:text-gray-300">
                <img id="moon" class="h-5 w-5" src="/assets/img/moon.png"/>
                <img id="sun" class="h-5 w-5 hidden" src="/assets/img/day.png"/>
              </button>
            </li>
          </ul>
          <div class="hidden lg:block">
            <button v-on:click="isOpen = !isOpen" type="button" id="menu-button" aria-expanded="true" aria-haspopup="true" class="flex items-center">
              <img class="w-10 h-10 mr-2 rounded-full object-cover object-right" :src="options.find((option) => option.value === $i18n.locale).src" alt="">
              <select id="locale-select"  v-model="$i18n.locale" class="dark:bg-dark-pageBackgroundColor dark:text-gray-300 text-sm mr-3 mb-1">
                  <option v-for="option in options" v-bind:value="option.value" class="dark:text-gray-300" >
                    {{ option.text }}
                  </option>
                </select>
            </button>
